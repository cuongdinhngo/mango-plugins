...
(total + top cost driver) and, before it, runs a **dispatch-count gate** — it refuses to proceed when
the ledger has fewer rows than the run's dispatch count (the ledger's teeth; a **completeness** check
that blocks like an unfilled matrix column, never inspecting or cutting a row). The ledger is **facts
only — it never auto-cuts a check, a gate, a critic,
...
uncodified-standard item** into codify's provisional→ratify flow, not silently enforced. It costs
