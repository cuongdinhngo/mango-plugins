...
`PASS(render@<bp>)`, not a silent skip), and the **format-scope rule** (execute scopes the formatter
to the authored/edited files, never a wholesale reformat of a shared file). It costs tokens, so CI runs it only via the manual `eval.yml`
